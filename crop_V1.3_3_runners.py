...
# ! Draw ROI Function
def drawROI(fileName,page,page_no,roi_counter):
...
def extractquestion(page_no,qn_no,fileName,ssName,zoom,imgName, showImages):
    try:
        try:
        ...
        try:
            if (fileSaveError=="NO" ):
                ...
                # if(showMe=="Y"):
                try:
                    if(showImages == "Y" or showImages == "y"):
                        if(imgName=="Y"):
...
def defaultRunner():
    ...
    print("")
    showImages = input("Do you want to see the images (Y/N) ")
    ...
    for page in range(pages):
        ...
        for q in range(q_done,questions):
            extractquestion(page, q,fileName,ssName,zoom,imgName, showImages)
            q_=q
        q_done=q_+1
    return
...
def manualCropROI():
    pdf_name = input("Enter name of PDF file = ")
    ...
    roi_counter = int(input("Enter the starting question number : "))
    ...
    for page_no in range(num_pages):
        ...
        while True:
            x, y, w, h = drawROI(pdf_name,page, page_no, roi_counter)
            ...
            try:
                mat = fitz.Matrix(zoom,zoom)
                if(imgName=="Y"):
                    page.get_pixmap(clip=(x0,y0,x1,y1),matrix=mat).save(f'{ssName}{roi_counter:03}.png')
                else:
                    page.get_pixmap(clip=(x0,y0,x1,y1),matrix=mat).save(f'{roi_counter}.png')
                try:
                    if(showImages == "Y" or showImages == "y"):
                        if(imgName=="Y"):
                            img = Image.open(f'{ssName}{roi_counter:03}.png')
                        else:
                            img = Image.open(f'{roi_counter}.png')
                ...
                print(Fore.RED+Style.BRIGHT+"")
                print("***************************** Warning !! *********************************")
                print(f"Unable to Save Image for Question {roi_counter}  || You need to crop it Manually")
                print("")
                print(Style.RESET_ALL+"")
                # return
...
try:
    ...
    print("    3. Manually Crop Using ROI                    ")  # 
    ...
    if(runner==1):
        ...
        CustomPageInput()
    elif runner == 3:  # If the user chooses option 3, run the PDF processing code
        print("YOU HAVE CHOOSE MANUAL CROPPING USING ROI")
        manualCropROI()
    else:
        print("YOU HAVE CHOOSE DEFAULT RUNNER")
        defaultRunner()
